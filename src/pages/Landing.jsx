...
import { ArrowRight, Star, Sparkles, Shield, Leaf, ChevronLeft, ChevronRight, Phone, Mail, Search, X, Heart } from 'lucide-react'

/* ─── Data ─────────────────────────────────────────────────── */
const features = [
  {
    icon: Sparkles,
    title: 'Crafted for African Hair',
    desc: 'Our formulations are developed specifically for the needs, textures, and styles of African hair — effective by design, not by chance.',
  },
  {
    icon: Leaf,
    title: 'Quality in Every Batch',
    desc: 'Consistent formulation standards and controlled production processes ensure every product performs exactly as expected.',
  },
  {
    icon: Shield,
    title: 'Reliable Manufacturing',
    desc: 'From careful raw material selection through to finished product, quality is built into every step of what we do.',
  },
]
...
const reviews = [
  {
    ...
    text: '"The Joi Moulding Gel Wax gives my clients the cleanest, longest-lasting styles. It has become a staple product in our salon."',
    stars: 5,
  },
  {
    name: 'Fatuma A.',
    role: 'Loyal Customer',
    text: '"The Leave-In Treatment has completely transformed my hair routine. Lightweight, actually effective, and smells amazing."',
    stars: 5,
  },
  {
    name: 'Brian O.',
    role: 'Distributor, Mombasa',
    text: '"Consistent quality and great packaging every time. Joi products practically sell themselves once customers try them."',
    stars: 5,
  },
]
...
function formatPrice(price) {
  ...
  return String(price).startsWith('Ksh') || String(price).startsWith('ksh') ? price : `Ksh ${price}`
}
...
function FloatingProduct({ img, size, top, left, right, bottom, delay, duration, rotate }) {
  ...
  return (
    <div style={{
      ...
      zIndex: 1,
      boxShadow: '0 20px 60px rgba(0,0,0,.12), inset 0 0 20px rgba(160,104,0,.08)',
      border: '2px solid rgba(140,90,0,.2)',
...
function ProductCard({ p, i, isActive, mobilePin }) {
  ...
  const cardInner = (
    <div
      ...
      style={{
        background: '#ffffff',
        border: `1px solid ${active ? 'rgba(140,90,0,.45)' : '#e0dbd0'}`,
        ...
        boxShadow: active
          ? '0 24px 60px rgba(0,0,0,.1), 0 0 30px rgba(160,104,0,.1)'
      ...
      >
        <Heart size={14} fill={liked ? '#e05050' : 'none'} color={liked ? '#e05050' : '#999'} />
      </button>
      ...
      <div style={{ position: 'relative', height: imgH, overflow: 'hidden', flexShrink: 0 }}>
        ...
        />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,.35) 0%, rgba(0,0,0,.1) 40%, transparent 65%)' }} />
        <div style={{
          position: 'absolute', top: 12, left: 12,
          padding: '5px 12px', background: 'rgba(255,255,255,.92)',
          border: '1px solid rgba(140,90,0,.3)',
          color: '#a06800', fontSize: 8, letterSpacing: '.16em',
          textTransform: 'uppercase',
          borderRadius: mobilePin ? 16 : 2,
        }}>
          {p.tag}
        </div>
        {!mobilePin && (
          <div style={{ position: 'absolute', bottom: 12, left: 12, color: 'rgba(255,255,255,.85)', fontSize: 9, letterSpacing: '.26em', textTransform: 'uppercase' }}>
            {p.cat}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: mobilePin ? '10px 10px 12px' : '1.2rem 1.3rem 1.4rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        {mobilePin && (
          <div style={{ fontSize: 7, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(140,90,0,.65)', marginBottom: 3 }}>
            {p.cat}
          </div>
        )}
        <h3 style={{
          ...
          color: '#1a1a1a', marginBottom: mobilePin ? 4 : 7,
          lineHeight: 1.3,
        }}>
          {p.name}
        </h3>
        {!mobilePin && (
          <p style={{ color: '#777', fontSize: 11.5, lineHeight: 1.75, marginBottom: 16, flex: 1 }}>
            {p.desc}
          </p>
        )}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 6, marginTop: mobilePin ? 'auto' : 0 }}>
          ...
          <button
            ...
            style={{
              ...
              background: active ? 'rgba(160,104,0,.08)' : 'transparent',
              border: '1px solid rgba(140,90,0,.35)',
              color: '#a06800', fontSize: mobilePin ? 8 : 9,
          ...
          >
            {added ? '✓' : <ArrowRight size={mobilePin ? 8 : 10} />}
            {!mobilePin && (added ? 'Added' : 'View')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto 44px', padding: '0 1rem' }}>
      <div style={{
        ...
        border: `1px solid ${focused ? 'rgba(140,90,0,.5)' : '#e0dbd0'}`,
        background: '#ffffff',
        transition: 'border .3s', display: 'flex', alignItems: 'center',
        boxShadow: focused ? '0 0 0 3px rgba(160,104,0,.08)' : 'none',
      }}>
        ...
        <input
          ...
          style={{ flex: 1, padding: '13px 14px', background: 'transparent', border: 'none', outline: 'none', color: '#1a1a1a', fontSize: 12, letterSpacing: '.04em', fontFamily: "'Montserrat',sans-serif" }}
        />
        {query && (
          <button onClick={() => setQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0 14px', color: 'rgba(140,90,0,.6)', display: 'flex', alignItems: 'center' }}>
            <X size={14} />
          </button>
        )}
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
        {allCats.map(cat => (
          <button key={cat} onClick={() => setActiveFilter(cat)} style={{
            padding: '7px 16px',
            background: activeFilter === cat ? 'rgba(160,104,0,.1)' : 'transparent',
            border: `1px solid ${activeFilter === cat ? 'rgba(140,90,0,.5)' : '#e0dbd0'}`,
            color: activeFilter === cat ? '#a06800' : '#888',
    ...
    <section ref={ref} style={{ padding: '4rem 0 6rem', position: 'relative', background: '#f7f6f2' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1rem', position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 36, flexWrap: 'wrap', gap: 14, opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(20px)', transition: 'all .7s ease' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[{ fn: prev, dis: idx === 0, Icon: ChevronLeft }, { fn: next, dis: idx >= products.length - 1, Icon: ChevronRight }].map(({ fn, dis, Icon }, ki) => (
              <button key={ki} onClick={fn} disabled={dis} style={{ width: 44, height: 44, border: `1px solid ${dis ? '#e0dbd0' : 'rgba(140,90,0,.4)'}`, background: 'transparent', cursor: dis ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: dis ? '#ccc' : '#a06800', transition: 'all .3s' }}>
                <Icon size={18} />
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 24 }}>
          {products.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)} style={{ width: i === idx ? 28 : 8, height: 8, borderRadius: 4, background: i === idx ? '#a06800' : 'rgba(140,90,0,.2)', border: 'none', cursor: 'pointer', transition: 'all .35s cubic-bezier(.25,.46,.45,.94)' }} />
          ))}
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 36 }}>
          ...
          >View Full Range <ArrowRight size={13} /></a>
        </div>
      </div>
    </section>
  )
  ...
  return (
    <section id="collections" ref={ref} style={{ padding: '5rem 1rem', position: 'relative', overflow: 'hidden', background: '#ffffff' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 1rem', color: '#aaa', fontFamily: "'Cormorant Garamond',serif", fontSize: '1.5rem' }}>
            ...
            {hasMore && (
              <div style={{ textAlign: 'center', marginTop: 28 }}>
                <button
                  ...
                  style={{
                    padding: '13px 36px', border: '1px solid rgba(140,90,0,.35)',
                    color: '#a06800', background: 'transparent', fontSize: 10,
    ...
    <section ref={ref} style={{ padding: '5rem 1rem', background: '#f7f6f2', borderTop: '1px solid #e8e2d8', borderBottom: '1px solid #e8e2d8', overflow: 'hidden', position: 'relative' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,260px),1fr))', gap: 18 }}>
          {reviews.map((r, i) => (
            <div key={r.name} style={{ padding: '2.2rem', border: `1px solid ${i === active ? 'rgba(140,90,0,.4)' : '#e8e2d8'}`, background: i === active ? '#ffffff' : '#faf9f6', opacity: vis ? 1 : 0, transform: vis ? 'translateY(0)' : 'translateY(30px)', transition: `all .7s ease ${i * .15}s, border .4s, background .4s`, boxShadow: i === active ? '0 8px 40px rgba(0,0,0,.07)' : 'none', cursor: 'pointer' }} onClick={() => setActive(i)}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 24 }}>
          {reviews.map((_, i) => (
            <button key={i} onClick={() => setActive(i)} style={{ width: i === active ? 24 : 8, height: 8, borderRadius: 4, background: i === active ? '#a06800' : 'rgba(140,90,0,.2)', border: 'none', cursor: 'pointer', transition: 'all .3s' }} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section ref={ref} style={{ padding: '6rem 1rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', textAlign: 'center', maxWidth: 580, margin: '0 auto', zIndex: 2, opacity: vis ? 1 : 0, transform: vis ? 'translateY(0)' : 'translateY(30px)', transition: 'all .85s ease' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 13, lineHeight: 1.9, marginBottom: 36 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          ...
          <a href="mailto:Wellssolutions2015@gmail.com" style={{ padding: '13px 36px', background: 'linear-gradient(135deg,#FFD700,#FFE34D)', color: '#1a1a1a', fontSize: 10, letterSpacing: '.22em', textTransform: 'uppercase', textDecoration: 'none', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 10, transition: 'transform .3s' }}
    ...
    <div style={{ background: '#ffffff', fontFamily: "'Montserrat',sans-serif" }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '9rem 1.2rem 3rem', maxWidth: 820 }}>
          <div style={{ fontSize: 9, letterSpacing: '.5em', textTransform: 'uppercase', color: '#e8c870', marginBottom: 20, opacity: 0, animation: 'landingFadeIn 1s ease .3s forwards' }}>Premium Hair &amp; Beauty Manufacturing · Proudly Made in Kenya</div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', opacity: 0, animation: 'landingFadeUp 1s ease .9s forwards' }}>
            <a href="#collections" style={{ padding: '13px 32px', background: 'linear-gradient(135deg,#FFD700,#FFE34D)', color: '#1a1a1a', fontSize: 10, letterSpacing: '.22em', textTransform: 'uppercase', textDecoration: 'none', fontWeight: 600, display: 'inline-block', transition: 'transform .3s' }}
              onMouseEnter={e => (e.currentTarget.style.transform = 'translateY(-2px)')}
              onMouseLeave={e => (e.currentTarget.style.transform = 'none')}>Explore Products</a>
            <a href="#story" style={{ padding: '13px 32px', background: 'transparent', border: '1px solid rgba(255,215,0,.6)', color: '#FFD700', fontSize: 10, letterSpacing: '.22em', textTransform: 'uppercase', textDecoration: 'none', transition: 'all .3s' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,215,0,.1)'; e.currentTarget.style.borderColor = 'rgba(255,215,0,.9)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(255,215,0,.6)' }}>Our Story</a>
          </div>
        </div>
      ...
      {/* ── Features ── */}
      <section ref={featRef} id="story" style={{ padding: '4.5rem 1rem', maxWidth: 1280, margin: '0 auto', position: 'relative', overflow: 'hidden' }}>
        <Features />
      </section>
